Feature: TC-LSR-11 Test Case
  NFS-15672 - Lessor - Validate Accounting TC-LSR-11

  Background: User needs to login into the application before performing the test case
    Given User logins into the NCP cockpit with correct credentials
    Then User opens the "nakisa-financial-suite" application url

  #TC_Title:Lessor - Validate Accounting TC-LSR-11
  #Jira_ID:NFS-15672
  ...
  Scenario Outline: TC-LSR-11 Test Case
    ...
    Then User tries to create Batch Posting Job
      ...
      |       1000 | Lessor             | Post             | Erp System's Default | Accrual,Depreciation,Payment | Open             | User Defined   | 2020-01-01 | User Defined | 2020-06-30 | Schedule Period Dates | Yes         | Activation Group |
    ...
    #--------------------Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Post" the "Accrual" of "OperatingLease" Schedules from "2020-01-01" to "2020-06-30"
    Then User clicks on "GAAP" Standard to "Post" the "Accrual" of "OperatingLease" Schedules from "2020-01-01" to "2020-06-30"
    #-------------------- Liability Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Post" the "Payment" of "Payment" Schedules from "2020-01-01" to "2020-06-30"
    Then User clicks on "GAAP" Standard to "Post" the "Payment" of "Payment" Schedules from "2020-01-01" to "2020-06-30"
    #--------------------Lease Component Event Level--------------------------
    Then User click on the "Lease Component" with Name "TC-LSR-11"
    ...
    Then User exercise the Term and Condition number "1,2,3,4,5,6"
    ...
    #Then User tries to create Batch Posting Job
    #| Batch Size | Principal Position | Transaction Type | Reversal Reason      | Journal Types                | Posting Statuses | From Date Type | From Date  | To Date Type | To Date               | Posting Date Type | Open Drafts      | Entity Type |
    #| 1000       | Lessor             | Post             | Erp System's Default | Accrual,Depreciation,Payment | Open             | User Defined   | 2020-07-01 | User Defined | 2021-12-31   | Schedule Period Dates | Yes               | Activation Group |
    #Then the "Batch" job should be completed with status "Done"
      #--------------------Search With ID--------------------------
    #Then User Search "Activation Group" with ID
    ...
    #--------------------Schedule Posting--------------------------
    #Then User clicks on "IAS" Standard to "Post" the "Accrual" of "OperatingLease" Schedules from "2020-07-01" to "2021-12-31"
    #Then User clicks on "GAAP" Standard to "Post" the "Accrual" of "OperatingLease" Schedules from "2020-07-01" to "2021-12-31"
    #-------------------- Liability Schedule Posting--------------------------
    #Then User clicks on "IAS" Standard to "Post" the "Payment" of "Payment" Schedules from "2020-07-01" to "2021-12-31"
    #Then User clicks on "GAAP" Standard to "Post" the "Payment" of "Payment" Schedules from "2020-07-01" to "2021-12-31"
