Feature: NFS-11237 Test Case
  NFS-11237 - Salvage Value - Verify Salvage value field is added on LC, AG.

  Background: User needs to login into the application before performing the test case
    Given User logins into the NCP cockpit with correct credentials
    Then User opens the "nakisa-financial-suite" application url

  #TC_Title:Salvage Value - Verify Salvage value field is added on LC, AG.
  #Jira_ID:NFS-11237
  #TC_Category:Regression
  #TC_Customers:None
  #TC_FixVersion:Nakisa 2023.R4 & Earlier
  @Regression @Xray @Core-Functionality
  Scenario Outline: NFS-11237 Test Case
    #--------------------Reading Test Case Excel--------------------------
    Given Reading test case inputs from excel file "<testCaseNumber>"
    #--------------------Master Agreement Level--------------------------
    Then User tries to create Master Agreement
    Then User sends the "Master Agreement" for "mla-send-to-approval-btn" workflow transition
    Then User sends the "Master Agreement" for "mla-approve-btn" workflow transition
    #--------------------Contract Level--------------------------
    And User tries to create Contract
    Then User answers all the questions
    Then User enters data under Contract Definition page
    Then User selects partner role and partner under Contract Partner page at "Inception" Level
    Then User fills the Accounting Tab of Contract
    Then User sends the "Contract" for "contract-send-to-approval-btn" workflow transition
    Then User sends the "Contract" for "contract-approve-btn" workflow transition
    #--------------------Lease Component Level--------------------------
    And User tries to create Lease Component
    Then User enters data under Lease Component Definition page
    And User add new term and condition at "Inception" level
    And User sends the "Lease Component" for "lc-send-to-approval-btn" workflow transition
    And User sends the "Lease Component" for "lc-approve-btn" workflow transition
    #--------------------Activation Group Level--------------------------
    Then User enters the "Inception" Definition Page values of Activation Group
    Then User enters the Terms and Conditions tab of Activation Group
    Then User exercise the Term and Condition number "1"
    Then User enters the Accounting tab of Activation Group
    Then User enters the "Inception" Consumer Price Index Values of Activation Group
    Then User enters the Terms and Conditions tab of Activation Group
    Then User index the Term and Condition number "1"
    Then User Completes the Activation Group Unit List
    And User sends the "Activation Group" for "ag-generate-schedules-btn" workflow transition
    And User sends the "Activation Group" for "ag-send-to-assessment-btn" workflow transition
    And User sends the "Activation Group" for "ag-approve-btn" workflow transition
    And User sends the "Activation Group" for "ag-confirm-classification-btn" workflow transition
    And User sends the "Activation Group" for "ag-activate-btn" workflow transition
     #--------------------Lease Component Event Level--------------------------
    Then User click on the "Lease Component" with Name "NFS-11237"
    Then User adds a New LC Event "Add new Base Rent"
    Then User verify that "Salvage Value" is "Disable"
    And User add new term and condition at "LC Event-1" level
    And User sends the "Lease Component Event" for "lc-send-to-approval-btn" workflow transition
    And User sends the "Lease Component Event" for "lc-approve-btn" workflow transition
    #--------------------Activation Group 1-Terms & Conditions Reassessment Event------------------
    Then User tries to create AG Event at "AG Event-1" Level
    Then User enters the "AG Event-1" Definition Page values of Activation Group
    Then User enters the Terms and Conditions tab of Activation Group
    Then User exercise the Term and Condition number "2"
    And User sends the "Activation Group" for "ag-generate-schedules-btn" workflow transition
    And User sends the "Activation Group" for "ag-send-to-assessment-btn" workflow transition
    And User sends the "Activation Group" for "ag-approve-btn" workflow transition
    And User sends the "Activation Group" for "ag-confirm-classification-btn" workflow transition
    And User sends the "Activation Group" for "ag-activate-btn" workflow transition
     #--------------------Activation Group 2-Indexation Event------------------
    Then User tries to create AG Event at "AG Event-2" Level
    Then User verify that "Salvage Value" is "Disable"
    #--------------------Activation Group 3-Contract Rate Change Event------------------
    Then User clicks on "Active" status "Activation Group" with name "LeaseModification-1"
    Then User tries to create AG Event at "AG Event-3" Level
    Then User verify that "Salvage Value" is "Disable"
    #--------------------Activation Group 4-Impairment Loss Event------------------
    Then User clicks on "Active" status "Activation Group" with name "LeaseModification-1"
    Then User tries to create AG Event at "AG Event-4" Level
    Then User verify that "Salvage Value" is "Disable"
    And User sends the "Activation Group" for "ag-generate-schedules-btn" workflow transition
    And User sends the "Activation Group" for "ag-send-to-assessment-btn" workflow transition
    And User sends the "Activation Group" for "ag-approve-btn" workflow transition
    And User sends the "Activation Group" for "ag-confirm-classification-btn" workflow transition
    And User sends the "Activation Group" for "ag-activate-btn" workflow transition
     #--------------------Activation Group 5-Impairment Gain Event------------------
    Then User tries to create AG Event at "AG Event-5" Level
    Then User verify that "Salvage Value" is "Disable"
    #--------------------Activation Group 6-Decrease in Term Event------------------
    Then User clicks on "Active" status "Activation Group" with name "ImpairmentLoss-4"
    Then User tries to create AG Event at "AG Event-6" Level
    Then User verify that "Salvage Value" is "Disable"
    #--------------------Activation Group 7-Decrease in Assets Event------------------
    Then User clicks on "Active" status "Activation Group" with name "ImpairmentLoss-4"
    Then User tries to create AG Event at "AG Event-7" Level
    Then User verify that "Salvage Value" is "Disable"


    Examples: 
      | testCaseNumber |
      | NFS-11237      |
