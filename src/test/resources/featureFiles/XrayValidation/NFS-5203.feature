Feature: NFS-5203 Test Case
  NFS-5203 - Mass Indexation: Add Job Report Details Popup/Batch-Indexation Jobs (CPI Global) - Verify that if the Indexation
  task has failed items then user will see a Show Report button on the right of the task

  Background: User needs to login into the application before performing the test case
    Given User logins into the NCP cockpit with correct credentials
    Then User opens the "nakisa-financial-suite" application url

  #TC_Title:Verify that if Indexation (CPI Global) task has failed items then user will see a Report button
  #Jira_ID:NFS-5203
  #TC_Category:Regression
  #TC_Customers:None
  #TC_FixVersion:Nakisa 2023.R4 & Earlier
  @Regression @Xray @Core-Functionality
  Scenario Outline: NFS-5203 Test Case
    #--------------------Reading Test Case Excel-------------------------- CPI-Global (Lease)
    ...
    Then User tries to create Indexation Posting Job
      ...
      |        100 | Lessee             | User Defined           | 2023-06-01        | CPI Global              | null                     | null                    | Modification Date           | null                   | Enforce Non-Lease         | Modification Date            | null                    | null                        | null                         | null                        | null                            | null                       | null                             | null                        | Modification Date | null         | null          | All                | null                 | All                 | null           | Yes         | Activation Group |
    Then the "Batch" job should be completed with status "Done"
      #---------------------Verification of Mass Indexation Job---------------------------------#
    Then User verify the Status of "Mass Indexation Job"
