Feature: NFS-6837 Test Case
  NFS-6837: NLA: Approval and Rejection comments- Verify that user should be allowed to
  enter approval or rejection comments (Approver Comment at rework at Inception level Scenario)

  Background: User needs to login into the application before performing the test case
    Given User logins into the NCP cockpit with correct credentials
    Then User opens the "nakisa-financial-suite" application url

  #TC_Title:Approver Comment at rework at Inception level Scenario
  #Jira_ID:NFS-6837
  #TC_Category:Regression
  #TC_Customers:None
  #TC_FixVersion:Nakisa 2023.R4 & Earlier
  @Regression @Xray @Core-Functionality
Scenario Outline: NFS-6837 Test Case
   #--------------------Reading Test Case Excel--------------------------(approver comment at rework at inception)
   Given Reading test case inputs from excel file "<testCaseNumber>"
   #--------------------Master Agreement Level--------------------------
   Then User tries to create Master Agreement
   Then User sends the "Master Agreement" for "mla-send-to-approval-btn" workflow transition
   Then User sends the "Master Agreement" for "mla-rework-btn" workflow with a message "MLA has been reworked"
   Then User validate the "Master Agreement" with a message "MLA has been reworked"
   Then User sends the "Master Agreement" for "mla-send-to-approval-btn" workflow transition
