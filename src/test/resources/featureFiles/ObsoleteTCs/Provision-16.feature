Feature: Provision-16 Test Case
  NFS-11928 - Verify the impact of provisioning classification on schedules: Scenario-16

  Background: User needs to login into the application before performing the test case
    Given User logins into the NCP cockpit with correct credentials
    Then User opens the "nakisa-financial-suite" application url

  #TC_Title:Verify the impact of provisioning classification on schedules: Scenario-16
  #Jira_ID:NFS-11928
  #TC_Category:CustomerUAT
  #TC_Customers:Sephora
  #TC_FixVersion:Nakisa 2023.R4 & Earlier
#  @CustomerUAT @provision
  Scenario Outline: Provision-16 Test Case
    ...
    #--------------------Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Post" the "Provision" of "Provision" Schedules from "2022-01-01" to "2022-06-30"
    Then User clicks on "GAAP" Standard to "Post" the "Accrual & Depreciation" of "AllColumns" Schedules from "2022-01-01" to "2022-06-30"
    #--------------------Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Post" the "ProvisionPayment" of "ProvisionPayment" Schedules from "2022-01-01" to "2022-06-30"
    ...
    And User sends the "Activation Group" for "ag-activate-btn" workflow transition
    #--------------------All Column Schedule Validation--------------------------
    And User clicks on "IAS" standard to validate "Provision" schedule view at "Event1-CA" level
    And User clicks on "GAAP" standard to validate "AllColumns" schedule view at "Event1-CA" level
    #--------------------Liability Schedule Validation--------------------------
    And User clicks on "IAS" standard to validate "ProvisionPayment" schedule view at "Event1-CA" level
    And User clicks on "GAAP" standard to validate "Liability" schedule view at "Event1-CA" level
    #--------------------Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Post" the "Provision" of "Provision" Schedules from "2022-07-01" to "2022-07-31"
    Then User clicks on "GAAP" Standard to "Post" the "Accrual & Depreciation" of "AllColumns" Schedules from "2022-07-01" to "2022-07-31"
    #--------------------Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Post" the "ProvisionPayment" of "ProvisionPayment" Schedules from "2022-07-01" to "2022-07-31"
