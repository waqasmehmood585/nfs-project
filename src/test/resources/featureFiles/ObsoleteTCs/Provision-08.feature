Feature: Provision-08 Test Case
  NFS-11912 - Verify the impact of provisioning classification on schedules: Scenario-08

  Background: User needs to login into the application before performing the test case
    Given User logins into the NCP cockpit with correct credentials
    Then User opens the "nakisa-financial-suite" application url

  #TC_Title:Verify the impact of provisioning classification on schedules: Scenario-08
  #Jira_ID:NFS-11912
  #TC_Category:CustomerUAT
  #TC_Customers:Sephora
  #TC_FixVersion:Nakisa 2023.R4 & Earlier
#  @CustomerUAT @provision
  Scenario Outline: Provision-08 Test Case
    ...
    #--------------------Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Post" the "Provision" of "Provision" Schedules from "2022-01-01" to "2022-09-30"
    Then User clicks on "GAAP" Standard to "Post" the "Accrual & Depreciation" of "AllColumns" Schedules from "2022-01-01" to "2022-09-30"
    #--------------------Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Post" the "ProvisionPayment" of "ProvisionPayment" Schedules from "2022-01-01" to "2022-09-30"
    Then User clicks on "GAAP" Standard to "Post" the "Payment" of "Liability" Schedules from "2022-01-01" to "2022-09-30"
    ...
    Then User click on the "Lease Component" with Name "Provision-08"
    ...
    And User makes the Rou End Date Change To "2023-04-15"
    And User sends the "Activation Group" for "ag-generate-schedules-btn" workflow transition
    And User sends the "Activation Group" for "ag-send-to-assessment-btn" workflow transition
    And User sends the "Activation Group" for "ag-approve-btn" workflow transition
    And User sends the "Activation Group" for "ag-confirm-classification-btn" workflow transition
    ...
    #--------------------Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Post" the "Provision" of "Provision" Schedules from "2022-10-01" to "2023-04-30"
    Then User clicks on "GAAP" Standard to "Post" the "Accrual & Depreciation" of "AllColumns" Schedules from "2022-10-01" to "2023-04-30"
    #--------------------Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Post" the "ProvisionPayment" of "ProvisionPayment" Schedules from "2022-10-01" to "2023-04-30"
    Then User clicks on "GAAP" Standard to "Post" the "Payment" of "Liability" Schedules from "2022-10-01" to "2023-04-30"
    #--------------------Asset Transition Schedule Validation--------------------------
