Feature: TC-WM-SA-11 Test Case
  NFS-8475 - Verify Schedules for Accounting Test Case TC-WM-SA-11

  Background: User needs to login into the application before performing the test case
    Given User logins into the NCP cockpit with correct credentials
    Then User opens the "nakisa-financial-suite" application url

  #TC_Title:Verify Schedules for Accounting Test Case TC-WM-SA-11
  #Jira_ID:NFS-8475
  #TC_Category:CustomerUAT
  #TC_Customers:walmartSouthAfrica
  #TC_FixVersion:Nakisa 2023.R4 & Earlier
  @CustomerUAT @walmartSouthAfrica
  Scenario Outline: TC-WM-SA-11 Test Case
    ...
    Then User click on the "Lease Component" with Name "TC-WM-SA-11"
    ...
    Then User tries to create Batch Posting Job
      ...
      | 1000       | Lessee             | Post             | Erp System's Default | Payment       | Open             | User Defined   | 2022-08-01 | User Defined | 2024-12-01 | Schedule Period Dates | Yes         | Activation Group |
    Then the "Batch" job should be completed with status "Done"
    #--------------------Search With ID--------------------------
    Then User Search "Activation Group" with ID
    #--------------------Activation Group Decrease Lease Event--------------------------
    Then User tries to create AG Event at "AG Event-1" Level
    Then User enters the Terms and Conditions tab of Activation Group
    Then User exercise the Term and Condition number "1,2"
    And User makes the Rou End Date Change To "2026-12-27"
    And User sends the "Activation Group" for "ag-generate-schedules-btn" workflow transition
    And User sends the "Activation Group" for "ag-send-to-assessment-btn" workflow transition
    And User sends the "Activation Group" for "ag-approve-btn" workflow transition
    And User sends the "Activation Group" for "ag-confirm-classification-btn" workflow transition
...
#    Then User tries to create Batch Posting Job
#      | Batch Size | Principal Position | Transaction Type | Reversal Reason      | Journal Types                | Posting Statuses | From Date Type | From Date  | To Date Type | To Date    | Posting Date Type     | Open Drafts | Entity Type      |
#      | 1000       | Lessee             | Post             | Erp System's Default | Accrual,Depreciation,Payment | Open             | User Defined   | 2022-08-01 | User Defined | 2026-12-27 | Schedule Period Dates | Yes         | Activation Group |
   # Then the "Batch" job should be completed with status "Done"
#    #--------------------Search With ID--------------------------
#    Then User Search "Activation Group" with ID
