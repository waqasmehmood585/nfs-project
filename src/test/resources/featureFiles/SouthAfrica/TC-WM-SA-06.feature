Feature: TC-WM-SA-06 Test Case
  NFS-12484 - Verify Schedules for Accounting Test Case TC-WM-SA-06

  Background: User needs to login into the application before performing the test case
    Given User logins into the NCP cockpit with correct credentials
    Then User opens the "nakisa-financial-suite" application url

  #TC_Title:Verify Schedules for Accounting Test Case TC-WM-SA-06
  #Jira_ID:NFS-12484
  #TC_Category:CustomerUAT
  #TC_Customers:walmartSouthAfrica
  #TC_FixVersion:Nakisa 2023.R4 & Earlier
  @CustomerUAT @walmartSouthAfrica
  Scenario Outline: TC-WM-SA-06 Test Case
    ...
    Then User tries to create Batch Posting Job
      ...
      | 1000       | Lessee             | Post             | Erp System's Default | Accrual,Depreciation,Payment | Open             | User Defined   | 2022-08-01 | User Defined | 2024-04-28 | Schedule Period Dates | Yes         | Activation Group |
    ...
    #--------------------Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Validate" the "Accrual & Depreciation" of "AllColumns" Schedules from "2022-08-01" to "2022-09-05"
    Then User clicks on "GAAP" Standard to "Validate" the "Accrual & Depreciation" of "AllColumns" Schedules from "2022-08-01" to "2022-09-05"
    #-------------------- Liability Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Validate" the "Payment" of "Liability" Schedules from "2022-08-01" to "2022-09-05"
    Then User clicks on "GAAP" Standard to "Validate" the "Payment" of "Liability" Schedules from "2022-08-01" to "2022-09-05"
    ...
    Then User tries to create Batch Posting Job
      | Batch Size | Principal Position | Transaction Type | Reversal Reason      | Journal Types                | Posting Statuses | From Date Type | From Date  | To Date Type | To Date    | Posting Date Type     | Open Drafts | Entity Type      |
      | 1000       | Lessee             | Post             | Erp System's Default | Accrual,Depreciation,Payment | Open             | User Defined   | 2024-04-29 | User Defined | 2024-12-01 | Schedule Period Dates | Yes         | Activation Group |
    Then the "Batch" job should be completed with status "Done"
    #--------------------Search With ID--------------------------
    Then User Search "Activation Group" with ID
    #--------------------Journal Document Validation--------------------------
    ...
    #--------------------Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Validate" the "Accrual & Depreciation" of "AllColumns" Schedules from "2024-04-29" to "2024-06-30"
    Then User clicks on "GAAP" Standard to "Validate" the "Accrual & Depreciation" of "AllColumns" Schedules from "2024-04-29" to "2024-06-30"
    #-------------------- Liability Schedule Posting--------------------------
    Then User clicks on "IAS" Standard to "Validate" the "Payment" of "Liability" Schedules from "2024-04-29" to "2024-06-30"
    Then User clicks on "GAAP" Standard to "Validate" the "Payment" of "Liability" Schedules from "2024-04-29" to "2024-06-30"
    #--------------------Asset Transition Schedule Validation--------------------------
